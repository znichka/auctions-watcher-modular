...
public class TelegramBotSender {
    ...
    public void sendImageUpload(TelegramBotCredentials credentials, byte[] photoContent, String photoCaption, String itemUrl) throws IOException {
        ...
        try {
            ...
            if (e.getStatusCode().value() == 429) {
                throw new TelegramRateLimitException(itemUrl, parseRetryAfterSeconds(e.getResponseBodyAsString()), e);
            }
            throw new IOException(String.format("Telegram API returned %s: %s", e.getStatusCode(), e.getResponseBodyAsString()), e);
        } catch (RestClientException e) {
            throw new IOException("Error while calling Telegram API for item " + itemUrl, e);
        }
    }

    private Integer parseRetryAfterSeconds(String responseBody) {
        try {
            JsonNode retryAfter = objectMapper.readTree(responseBody).path("parameters").path("retry_after");
            return retryAfter.isMissingNode() ? null : retryAfter.asInt();
        } catch (IOException e) {
            return null;
        }
    }
